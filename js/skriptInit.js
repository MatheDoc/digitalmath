...
const mathjaxReady = new Promise((res) =>
  document.addEventListener("mathjax-ready", res)
);
...
Promise.all([domReady, mathjaxReady])
  ...
  .then((markdownText) => {
    ...
    script.onerror = () => {
      console.error(
        `Fehler: Das Skript für das Thema '${thema}' konnte nicht geladen werden.`
      );
    };
    ...
    bilder.forEach((img) => {
      ...
      if (originalSrc && !originalSrc.startsWith("http")) {
        img.src = `lernbereiche/${encodeURIComponent(
          thema
        )}/skript/${originalSrc}`;
      }
    });
